...
public class Msg {
    ...
    public static void info(CommandSender to, String msg, Object... args) {
        to.sendMessage(format("&r[&3PokeMob&r] ") + format(msg, args));
    }

    public static void warn(CommandSender to, String msg, Object... args) {
        to.sendMessage(format("&r[&cPokeMob&r] &c") + format(msg, args));
    }

    static void consoleCommand(String cmd, Object... args)
    {
        if (args.length > 0) cmd = String.format(cmd, args);
        if (PokeMobPlugin.getInstance().getConfiguration().isDebugMode()) {
            PokeMobPlugin.getInstance().getLogger().info("Running Console Command: " + cmd);
        }
